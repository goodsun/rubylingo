...
class JMdictParser {
    ...
    while ((match = entryRegex.exec(xmlContent)) !== null) {
      try {
        ...
        if (entryCount % 10000 === 0) {
          process.stdout.write(`\r処理中: ${entryCount.toLocaleString()} エントリ`);
        }
  ...
  parseEntry(entryXml) {
    ...
    const translations = glossMatches.map(m => m[1].trim()).filter(t => t.length > 0);
  ...
  estimateFrequency(word, translations) {
    ...
    const commonKanji = ['人', '日', '本', '国', '時', '年', '手', '目', '心', '水', '火', '金', '木', '土', '山', '川', '田'];
    if (commonKanji.some(k => word.includes(k))) score += 20;

    // Business/academic common words
    const businessWords = ['会議', '重要', '戦略', '企業', '経済', '技術', '開発', '管理', '営業', '販売'];
    if (businessWords.includes(word)) score += 40;

    // Basic verbs and adjectives  
    const basicWords = ['する', 'ある', 'いる', 'なる', '言う', '思う', '見る', '来る', '行く', '出る', '入る', '大きい', '小さい', '新しい', '古い', '良い', '悪い', '高い', '安い'];
  ...
  async generateDictionaries() {
    ...
    const sortedEntries = this.entries.sort((a, b) => b.frequency - a.frequency);
    ...
    for (const [level, config] of Object.entries(levels)) {
      ...
      console.log(`  ${level}: ${entries.length.toLocaleString()}語 -> ${outputPath}`);
    }
